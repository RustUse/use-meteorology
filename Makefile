...
FOCUSED_CRATES := use-weather-observation use-atmosphere use-air-temperature use-atmospheric-pressure use-humidity use-wind use-cloud use-precipitation use-weather-front use-pressure-system use-weather-forecast
...
help:
	@printf "%s\n" \
		...
		"deny                           Run cargo-deny" \
		"sbom                           Generate a CycloneDX SBOM for use-meteorology" \
		"publish-dry-run-focused        List package contents and dry-run publish focused crates" \
		"publish-dry-run-facade         Dry-run publish use-meteorology after crates.io propagation" \
...
sbom:
	cargo cyclonedx --manifest-path crates/use-meteorology/Cargo.toml --all-features --format json --spec-version 1.5 --override-filename sbom.cyclonedx
...
publish-dry-run-facade:
	cargo publish --dry-run --allow-dirty -p use-meteorology
